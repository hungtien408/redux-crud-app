...
const photo = createSlice({
  ...
  reducers: {
    ...
    updatePhoto: (state, action) => {
      const newPhoto = action.payload;
      const idxPhoto = state.findIndex((x) => x.id === newPhoto.id);

      if (idxPhoto >= 0) {
        state[idxPhoto] = newPhoto;
      }
    },
    removePhoto: (state, action) => {
      const photoId = action.payload;
      // const idxPhoto = state.findIndex((x) => x.id === photoId);

      // if (idxPhoto >= 0) {
      //   state.splice(idxPhoto, 1);
      // }
      return state.filter((x) => x.id !== photoId);
    },
  },
});
